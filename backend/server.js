...
app.get('/api/export-history', (req, res) => {
    // 1. Set headers so the browser downloads the file
    const format = req.query.format === 'csv' ? 'csv' : 'json';
    res.setHeader('Content-Disposition', `attachment; filename="TradeReport.${format}"`);
    res.setHeader('Content-Type', format === 'csv' ? 'text/csv' : 'application/json');

    // 2. Create the stream from the database path defined above
    const readStream = fs.createReadStream(dbPath);

    // 3. Pipe the data directly to the response
    // This uses almost zero RAM because data isn't stored in a variable
    readStream.pipe(res);

    // 4. Error handling for the stream
